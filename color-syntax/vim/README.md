...
This guide works for **VIM** and **NEOVIM** !



## 1) Go to your vim configuration folder
...
## 3) Configure `.sync` file detection

- Create a `.ftdetect/` and inside create a `sync.vim` file and write :

```vim
au BufRead,BufNewFile *.sync set filetype=sync
```

This will make VIM detect your `.sync` files.

## 4) Create a syntax file

- Create a `syntax/` and inside put the [sync.vim](./sync.vim) file

<details>
    <summary>Or you can copy it from here</summary>
