...
class Shape(ABC):
    ...
    def keyframes(self):
        ...
        if not self._sorted:
            ...
            for key in self._keyframes.keys():
                ...
                self._keyframes[key] = dict(
                    sorted(
                        self._keyframes[key].items(),
                        key=lambda keyvalue: keywords_key(keyvalue[0]),
                    )
                )

            # Sort by key
            self._keyframes = defaultdict(dict, sorted(self._keyframes.items()))
            self._sorted = True
    ...
    def to_svg(self):
        ...
        s = (
            '<%s %s id="%s" stroke="%s" stroke-width="%d" fill="%s" '
...
class Oval(Shape):
    ...
    def attributes_svg(self):
        return 'cx="%d" cy="%d" rx="%d" ry="%d"' % (self.cx, self.cy, self.rx, self.ry)
...
class PolyLine(Shape):
    ...
    def coordinates_str(self):
        ...
        return "  ".join(
            "%d,%d" % (x, y)
            for x, y in zip(self._coordinates[::2], self._coordinates[1::2])
        )
    ...
    def _load_coordinates(self, turn):
        ...
        if coords is None:
            return
        self.coordinates = [
            int(x) for x in re.split(r"[, ]", coords) if re.match(r"-?\d+", x)
        ]
...
class RegularPolygon(PolyLine):
    ...
    def __init__(self, name, cx, cy, radius, nb_edges):
        ...
        for i in range(1, (nb_edges + 1) // 2):
            angle = -math.pi / 2 + i * alpha
            delta_x = radius * math.cos(angle)
            delta_y = radius * math.sin(angle)
            coords.append(cx + delta_x)
            coords.append(cy + delta_y)
            anti_coords.append(cy + delta_y)
            anti_coords.append(cx - delta_x)

        if nb_edges % 2 == 0:
            anti_coords.append(cy + radius)
            anti_coords.append(cx)

        coords = coords + list(reversed(anti_coords))
...
class Text(Shape):
    ...
    def attributes_svg(self):
        return (
            'x="%d" y="%d" dominant-baseline="%s" text-anchor="%s" font-family="%s" font-size="%d"'
            % (
                self.x,
                self.y,
                self.vertical_align,
...
class Path(Shape):
    ...
    def add_horizontal_line_element(self, absolute, x):
        ...
        self._elements.append(
            Path.PathElement(Path.PathElement.HORIZONTAL, absolute, [x])
        )
    ...
    def add_vertical_line_element(self, absolute, y):
        ...
        """
        self._elements.append(
            Path.PathElement(Path.PathElement.VERTICAL, absolute, [y])
        )
    ...
    def add_bezier_curve_element(self, absolute, x1, y1, x2, y2, x, y):
        ...
        """
        self._elements.append(
            Path.PathElement(Path.PathElement.CURVE, absolute, [x1, y1, x2, y2, x, y])
        )
    ...
    def add_short_bezier_curve_element(self, absolute, x2, y2, x, y):
        ...
        """
        self._elements.append(
            Path.PathElement(Path.PathElement.SHORT_CURVE, absolute, [x2, y2, x, y])
        )
    ...
    def add_quadratic_bezier_curve_element(self, absolute, x1, y1, x, y):
        ...
        """
        self._elements.append(
            Path.PathElement(Path.PathElement.QUADRATIC, absolute, [x1, y1, x, y])
        )
    ...
    def add_short_quadratic_bezier_curve_element(self, absolute, x, y):
        ...
        """
        self._elements.append(
            Path.PathElement(Path.PathElement.SHORT_QUADRATIC, absolute, [x, y])
        )

    def add_arc_element(
        self, absolute, rx, ry, x_axis_rotation, large_arc_flag, sweep_flag, x, y
    ):
        """
    ...
    def set_move_element(self, index, absolute, x, y):
        ...
        if index < 1 or index >= len(self._elements):
            return
        self._elements[index] = Path.PathElement(
            Path.PathElement.MOVE, absolute, [x, y]
        )
    ...
    def set_line_element(self, index, absolute, x, y):
        ...
        if index < 1 or index >= len(self._elements):
            return
        self._elements[index] = Path.PathElement(
            Path.PathElement.LINE, absolute, [x, y]
        )
    ...
    def set_horizontal_line_element(self, index, absolute, x):
        ...
        if index < 1 or index >= len(self._elements):
            return
        self._elements[index] = Path.PathElement(
            Path.PathElement.HORIZONTAL, absolute, [x]
        )
    ...
    def set_vertical_line_element(self, index, absolute, y):
        ...
        if index < 1 or index >= len(self._elements):
            return
        self._elements[index] = Path.PathElement(
            Path.PathElement.VERTICAL, absolute, [y]
        )
    ...
    def set_bezier_curve_element(self, index, absolute, x1, y1, x2, y2, x, y):
        ...
        if index < 1 or index >= len(self._elements):
            return
        self._elements[index] = Path.PathElement(
            Path.PathElement.CURVE, absolute, [x1, y1, x2, y2, x, y]
        )
    ...
    def set_short_bezier_curve_element(self, index, absolute, x2, y2, x, y):
        ...
        if index < 1 or index >= len(self._elements):
            return
        self._elements[index] = Path.PathElement(
            Path.PathElement.CURVE, absolute, [x2, y2, x, y]
        )
    ...
    def set_quadratic_bezier_curve_element(self, index, absolute, x1, y1, x, y):
        ...
        if index < 1 or index >= len(self._elements):
            return
        self._elements[index] = Path.PathElement(
            Path.PathElement.QUADRATIC, absolute, [x1, y1, x, y]
        )
    ...
    def set_short_quadratic_bezier_curve_element(self, index, absolute, x, y):
        ...
        if index < 1 or index >= len(self._elements):
            return
        self._elements[index] = Path.PathElement(
            Path.PathElement.SHORT_QUADRATIC, absolute, [x, y]
        )

    def set_arc_element(
        self, index, absolute, rx, ry, x_axis_rotation, large_arc_flag, sweep_flag, x, y
    ):
        """
