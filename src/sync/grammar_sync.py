...
class SynCParser(lrparsing.Grammar):
    class T(lrparsing.TokenRegistry):
        ...
        libfunctions = Tokens(
            "IABS FABS COS SIN TAN SQRT EXP LN POW SH CH TH CEIL FLOOR ROUND MALLOC MALLOC_STRUCT FREE "
    ...
    # Appel de fonction
    call = (
        ((T.ident | T.libfunctions) + "(" + List(_expr, ",") + ")")
        | (
            Tokens("PRINT PRINTERR")
            + "("
            + List((_expr | Tokens("INT FLOAT BOOL POINTER STRING CHAR")), ",")
            + ")"
        )
    ...
    # Littéraux atomiques
    atom = (
        T.ident | T.integer | T.floatn | T.char | T.string | Tokens("TRUE FALSE NULL")
    )
    ...
    ifexpr = (
        ...
        + ")"
        + block
        + Many(Token("ELIF") + "(" + _expr + ")" + block)
        + Opt(Token("ELSE") + block)
        + Token("ENDIF")
    )
    forexpr = (
        Token("FOR")
    ...
    globalaffect = (
        T.ident
        + "="
        + (T.integer | T.floatn | T.char | T.string | Tokens("TRUE FALSE NULL"))
    )
...
SYNC_AST_CHILDREN_INDEXES = {
    ...
    SynCParser.whileexpr: {
        "condition_expr": 3,
        "block": 5,
        "line": [1, -2],
        "char": [1, -1],
    },
    SynCParser.globalaffect: {
        "variable": 1,
        "value": 3,
        "line": [2, -2],
        "char": [2, -1],
    },
    SynCParser.globalsdecl: {
        "global_variables": [2, -1, 1],
        "line": [1, -2],
        "char": [1, -1],
    },
    SynCParser.structdecl: {
        "struct_name": 2,
        "fields": [3, -1, 1],
        "line": [1, -2],
        "char": [1, -1],
