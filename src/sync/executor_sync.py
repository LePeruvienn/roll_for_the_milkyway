...
from src.sync.grammar_sync import (
    SynCParser,
...
class SynCProgram:
    ...
    def _check_memory_limit_exceeded(self, address=None):
        ...
        """
        local_variable_last_address = self.read_write_stack_limit + sum(
            len(l) for l in self.local_variables
        )
    ...
    def _get_variable_address(self, variable_name, line=None, char=None):
        ...
        try:
            ...
            address += len(self.global_variables) + sum(
                len(lv) for lv in self.local_variables[:-1]
            )
            try:
    ...
    def _get_string_from_address(self, address, line=None, char=None):
        ...
        while True:
            c = bin_to_char(
                self._get_memory(address, line=line, char=char), line=line, char=char
            )
            if c == "\0":
                return s
            s += c
    ...
    def _read_structs_and_functions(self, tree):
        ...
        def _read_structure(structure):
            ...
            structure_name = structure[
                SYNC_AST_CHILDREN_INDEXES[SynCParser.structdecl]["struct_name"]
            ][1]
            ...
            start, end, step = SYNC_AST_CHILDREN_INDEXES[SynCParser.structdecl][
                "fields"
            ]
        ...
        def _read_function(function):
            ...
            function_name = function[
                SYNC_AST_CHILDREN_INDEXES[SynCParser.fundecl]["function_name"]
            ][1]
            ...
            start, end, step = SYNC_AST_CHILDREN_INDEXES[SynCParser.fundecl][
                "parameters"
            ]
            ...
            for i, parameter in enumerate(parameters):
                if parameter in parameters[i + 1 :]:
                    raise SameParametersNameFunctionError(
                        node_line, node_char, function_name, parameter
                    )
            ...
            self.functions_infos[function[2][1]] = {
                ...
                "tree": function[
                    SYNC_AST_CHILDREN_INDEXES[SynCParser.fundecl]["block"]
                ],
                "line": node_line,
                "char": node_char,
            }
        ...
        def _search_for_static_strings_aux(subtree):
            ...
            # Si c'est une chaîne de caractère ...
                ...
                if string_value in self.read_only_strings_adress:
                    return
                self.read_only_strings_adress[string_value] = (
                    self.read_write_stack_limit
                )

                # On vérifie que la chaîne est en ASCII et les caractères échappés
                error_char_index = check_is_string_is_ascii_printable_with_escape(
                    string_value
                )
                if error_char_index is not None:
                    node_char += error_char_index + 1
                    raise NonValidCharacterError(
                        node_line, node_char, string_value[error_char_index]
                    )
    ...
    def _declare_globals(self, tree):
        ...
        start, end, step = SYNC_AST_CHILDREN_INDEXES[SynCParser.globalsdecl][
            "global_variables"
        ]
        for global_affect in globals_declaration[start:end:step]:
            variable = global_affect[
                SYNC_AST_CHILDREN_INDEXES[SynCParser.globalaffect]["variable"]
            ][1]
            value_literal = global_affect[
                SYNC_AST_CHILDREN_INDEXES[SynCParser.globalaffect]["value"]
            ]
            ...
            for function_name in self.functions_infos:
                ...
                if variable in parameters:
                    raise GlobalVariableAndFunctionParameterWithSameNameError(
                        node_line, node_char, variable, function_name
                    )
            ...
            if value_literal[0] == SynCParser.T.integer:
                ...
                raise ExecutionError(
                    node_line, node_char, "Unknown error CODE 0, contact developer"
                )

            address = self._get_variable_address(
                variable, line=node_line, char=node_char
            )
    ...
    def execute_function(self, function_name, *params):
        ...
        if len(parameters) != 0:
            address0 = self._get_variable_address(
                parameters[0], line=node_line, char=node_char
            )
    ...
    def execute_node(self, node):
        ...
        def _execute_atom():
            ...
            # un BINARY_KEYWORD ; la seule exception est le pointeur NULL ou une chaîne de caractère statique.
                ...
                # Ce cas ne devrait jamais se produire
                raise ExecutionError(
                    node_line, node_char, "Unknown error CODE 1, contact developer"
                )
        ...
        def _execute_affect():
            ...
            if (
                variable not in self.global_variables
                and variable not in self.local_variables[-1]
            ):
            ...
            for elementary, value, subnode_line, subnode_char in self.execute_node(
                expr
            ):
        ...
        def _execute_arrayaffect():
            ...
            # On effectue l'affectation
            self._set_memory(
                address + index_value, binary_value, line=node_line, char=node_char
            )
        ...
        def _execute_structaffect():
            ...
            # On effectue l'affectation
            self._set_memory(
                address + field_index, binary_value, line=node_line, char=node_char
            )
        ...
        def _execute_binaryoperator():
            ...
            if operator in ["+", "-", "/", "*", "%", "<=", ">=", "<", ">"]:
                ...
                if operator == "+":
                    ...
                    try:
                        result = int_to_bin(
                            left_value % right_value, overflow=True
                        )  # A priori, pas d'overflow possible
                    except ZeroDivisionError:
                        raise MathDomainError(node_line, node_char, right_value)
                elif operator == "<=":
                ...
                # Ce cas ne devrait jamais se produire
                raise ExecutionError(
                    node_line, node_char, "Unknown error CODE 2, contact developer"
                )
        ...
        def _execute_boolbinaryoperator():
            ...
            if operator == "AND":
                ...
                # Ce cas ne devrait jamais se produire
                raise ExecutionError(
                    node_line, node_char, "Unknown error CODE 3, contact developer"
                )
        ...
        def _execute_unaryoperator():
            ...
            if operator == "NOT":
                ...
                # Ce cas ne devrait jamais se produire
                raise ExecutionError(
                    node_line, node_char, "Unknown error CODE 4, contact developer"
                )
        ...
        def _execute_refexpr():
            ...
            address = self._get_variable_address(
                variable_name, line=node_line, char=node_char
            )
        ...
        def _execute_call():
            ...
            function_name = node[
                SYNC_AST_CHILDREN_INDEXES[SynCParser.call]["function_name"]
            ][1]
            ...
            if function_name in ["READ", "RAND", "FLUSH", "FLUSHERR"]:
                ...
                expected_nb_parameters = len(parameters) == len(
                    self.functions_infos[function_name]["parameters"]
                )

            if not expected_nb_parameters:
                raise IncorrectNumberOfParametersInFunction(
                    node_line, node_char, function_name
                )
            ...
            if function_name == "MALLOC_STRUCT":
                ...
                for parameter in parameters:
                    ...
                    for (
                        ...
                        subnode_line,
                        subnode_char,
                    ) in self.execute_node(parameter):
                        # On transfert si l'opération mais pas la valeur
                        yield elementary, None, subnode_line, subnode_char
            ...
            if function_name in [
                ...
                for i, binary_parameter in enumerate(binary_parameters[1:]):
                    self._set_memory(
                        address + i, binary_parameter, line=node_line, char=node_char
                    )
                    yield True, None, node_line, node_char
                ...
                pointer = bin_to_pointer(
                    binary_parameters[0], line=node_line, char=node_char
                )
                self._free(pointer)
            elif function_name in ["PRINT", "PRINTERR"]:
                ...
                for parameter in binary_parameters:
                    ...
                    parameter_is_type = parameter in [
                        "INT",
                        "FLOAT",
                        "BOOL",
                        "POINTER",
                    ...
                    if type is not None and parameter_is_type:
                        raise InvalidTypeParameterInFunctionError(
                            node_line, node_char, function_name, type
                        )
                    ...
                    if type is None or type == "STRING":
                        value = self._get_string_from_address(
                            bin_to_pointer(parameter, line=node_line, char=node_char),
                            line=node_line,
                            char=node_char,
                        )
                    elif type == "INT":
                        ...
                        value = bin_to_pointer(
                            parameter, line=node_line, char=node_char
                        )
                    else:
                        # Ce cas ne devrait jamais se produire
                        raise ExecutionError(
                            node_line,
                            node_char,
                            "Unknown error CODE 5, contact developer",
                        )
                ...
                # max de la chaîne
                value = bin_to_int(binary_parameters[0])
                address = bin_to_pointer(
                    binary_parameters[1], line=node_line, char=node_char
                )
                ...
                if parameter == "INT":
                    ...
                    # Ce cas ne devrait jamais se produire
                    raise ExecutionError(
                        node_line, node_line, "Unknown error CODE 6, contact developer"
                    )
            else:
                # Fonction custom
                for (
                    ...
                    subnode_line,
                    subnode_char,
                ) in self.execute_function(function_name, *binary_parameters):
                    # On transfert si l'opération mais pas la valeur
                    yield elementary, None, subnode_line, subnode_char
        ...
        def _execute_arraycall():
            ...
            result = self._get_memory(
                address + index_value, line=node_line, char=node_char
            )
        ...
        def _execute_structcall():
            ...
            result = self._get_memory(
                address + field_index, line=node_line, char=node_char
            )
        ...
        def _execute_ifexpr():
            ...
            condition_index = SYNC_AST_CHILDREN_INDEXES[SynCParser.ifexpr][
                "condition_expr"
            ]
            ...
            while token_index < len(node):
                ...
                if node[token_index][1] in ["IF", "ELIF"]:
                    ...
                    for (
                        ...
                        subnode_line,
                        subnode_char,
                    ) in self.execute_node(condition):
                        # On transfert si l'opération mais pas la valeur
                        yield elementary, None, subnode_line, subnode_char
                    ...
                    condition = bin_to_bool(
                        binary_condition, line=node_line, char=node_char
                    )
                ...
                if block is not None:
                    for (
                        ...
                        subnode_line,
                        subnode_char,
                    ) in self.execute_node(block):
                        yield elementary, value, subnode_line, subnode_char
                    return
                else:
                    token_index += SYNC_AST_CHILDREN_INDEXES[SynCParser.ifexpr][
                        "if_elif_delta"
                    ]
                    block_index += SYNC_AST_CHILDREN_INDEXES[SynCParser.ifexpr][
                        "if_elif_delta"
                    ]
                    condition_index += SYNC_AST_CHILDREN_INDEXES[SynCParser.ifexpr][
                        "if_elif_delta"
        ...
        def _execute_forexpr():
            ...
            """
            variable = node[
                SYNC_AST_CHILDREN_INDEXES[SynCParser.forexpr]["loop_variable"]
            ][1]
            ...
            start_expr = node[
                SYNC_AST_CHILDREN_INDEXES[SynCParser.forexpr]["start_expr"]
            ]
            ...
            self._set_memory(
                address, int_to_bin(start_expr), line=node_line, char=node_char
            )
            ...
            while (
                bin_to_int(self._get_memory(address, line=node_line, char=node_char))
                < end_expr
            ):
                ...
                self._set_memory(
                    ...
                    int_to_bin(
                        bin_to_int(
                            self._get_memory(address, line=node_line, char=node_char)
                        )
                        + 1
                    ),
                    line=node_line,
        ...
        def _execute_whileexpr():
            ...
            """
            condition_node = node[
                SYNC_AST_CHILDREN_INDEXES[SynCParser.whileexpr]["condition_expr"]
            ]
        ...
        def _execute_block():
            ...
            for blockexpr in node[start:end:step]:
                for elementary, value, node_line, node_char in self.execute_node(
                    blockexpr
                ):
                    yield elementary, value, node_line, node_char
...
def get_program_execution_iterator(program_filename=None, input_program=None):
    ...
    program = _get_program(
        program_filename=program_filename, input_program=input_program
    )
...
def execute_program(program_filename=None, input_program=None):
    ...
    :param input_program: une chaîne de caractère contenant un programme SynC, ignoré si program_filename est donné
    """
    for _ in get_program_execution_iterator(
        program_filename=program_filename, input_program=input_program
    ):
        pass
